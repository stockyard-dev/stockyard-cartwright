package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-cartwright/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-cartwright"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/dashboards",s.handleListDashboards)
    s.mux.HandleFunc("POST /api/dashboards",s.handleCreateDashboard)
    s.mux.HandleFunc("DELETE /api/dashboards/{id}",s.handleDeleteDashboard)
    s.mux.HandleFunc("GET /api/dashboards/{id}/charts",s.handleListCharts)
    s.mux.HandleFunc("POST /api/dashboards/{id}/charts",s.handleAddChart)
