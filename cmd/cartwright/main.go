...
db,err:=store.Open(dataDir);if err!=nil{log.Fatalf("cartwright: %v",err)};defer db.Close();srv:=server.New(db,server.DefaultLimits())
fmt.Printf("\n  Cartwright — Self-hosted dashboard and report builder\n  Dashboard:  http://localhost:%s/ui\n  API:        http://localhost:%s/api\n  Questions? hello@stockyard.dev — I read every message\n\n",port,port)
